...
const BookCard= ({book, setBooks})=>{
    ...
    const handleDelete = async()=>{
        ...
        return(
            <>
            {/*CARD*/}
            <Link to={`/book/${book._id}`} className={`relative block rounded-xl bg-base-100 p-4 boarder transition-all duration-200 ${isActive ? "boarder-primary shadow-lg": "border-base-300"}
            hover:border-primary hover:shadow-xl`}>
            {/*TOp Row*/}
            <div className="flex justify-between items-start">
                <p className="text-xs text-base-content/60 truncate">{book._id}</p>
                <span className="badge-secondary">{book.publishYear}</span>
            </div>
            {/*Book Info */}
            ...
            {showModal && (<dialog className="modal modal-open">
                <div className="modal-box">
                    ...
                    <span className="font-semibold text-base-content">
                        {" "}"{book.title}"
                        </span>? <br /> This action cannot be undo.</p>
